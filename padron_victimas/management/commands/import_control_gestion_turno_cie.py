...
class Command(BaseCommand):
    help = 'Importa registros de Turno CIE desde un archivo CSV.'
    ...
    def handle(self, *args, **options):
        ...
        with open(csv_path, encoding='utf-8') as f:
            ...
            for row in reader:
                try:
                    victima = Victima(
                        numero_registro=row.get('Núm_Registro', ''),
                        nombre=row.get('Víctimas_Relacionadas', ''),
                        apellido_paterno='',
                        apellido_materno='',
                        ...
                        email='',
                        direccion='',
                        tipo_victimizacion=row.get('Tipo', ''),
                        fecha_hechos=None,
                        lugar_hechos='',
                        estado='ACTIVO',
                        observaciones=row.get('Acuse_CIE', ''),
                        usuario_registro=usuario
                    )
                    victima.save()
                    count += 1
                except Exception as e:
                    self.stderr.write(f'Error en registro {row.get("Núm_Registro", ""):} - {e}')
