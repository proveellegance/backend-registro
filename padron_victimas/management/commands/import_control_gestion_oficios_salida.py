...
class Command(BaseCommand):
    help = 'Importa registros de Oficios de Salida desde un archivo CSV.'
    ...
    def handle(self, *args, **options):
        ...
        with open(csv_path, encoding='utf-8') as f:
            ...
            for row in reader:
                try:
                    victima = Victima(
                        numero_registro=row.get('Alfanumérica_Oficio', ''),
                        ...
                        telefono='',
                        email='',
                        direccion=row.get('Destinatario', ''),
                        tipo_victimizacion=row.get('Asunto', ''),
                        fecha_hechos=None,
                        lugar_hechos='',
                        estado='ACTIVO',
                        observaciones=row.get('Tipo_Envío', ''),
                        usuario_registro=usuario
                    )
                    victima.save()
                    count += 1
                except Exception as e:
                    self.stderr.write(f'Error en registro {row.get("Alfanumérica_Oficio", ""):} - {e}')
