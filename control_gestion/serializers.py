...
class OficioSalidaSerializer(serializers.ModelSerializer):
    ...
    def get_download_url(self, obj):
        ...
        if obj.archivo:
            request = self.context.get('request')
            if request:
                return request.build_absolute_uri(f'/api/control-gestion/oficios-salida/{obj.id}/descargar_pdf/')
        return None
    ...
    def get_preview_url(self, obj):
        ...
        if obj.archivo:
            request = self.context.get('request')
            if request:
                return request.build_absolute_uri(f'/api/control-gestion/oficios-salida/{obj.id}/vista_previa_pdf/')
        return None
